...
public class EditorController {
  ...
  public static int[] calculateStatistics(String text, String html) {
    int chars = text.length();
    int words = text.trim().isEmpty() ? 0 : text.trim().split("\\s+").length;
    int paragraphs = (html.split("<p>").length - 1);
    if (paragraphs == 0 && !text.isEmpty())
      paragraphs = 1;
    return new int[] { chars, words, paragraphs };
  }
  ...
  private void clearAllText() {
    ...
    alert.setHeaderText("Удалить весь текст?");
    alert.setContentText("Это действие нельзя отменить.");
    Optional<ButtonType> result = alert.showAndWait();
    if (result.isPresent() && result.get() == ButtonType.OK) {
      webEngine.executeScript("document.getElementById('content').innerHTML = ''");
      showInfo("Очистка", "Текст удалён");
    }
  }
  ...
  private void insertDate() {
    ...
    webEngine.executeScript("document.execCommand('insertText', false, '" + date + "')");
    showInfo("Дата", "Вставлена текущая дата");
  }

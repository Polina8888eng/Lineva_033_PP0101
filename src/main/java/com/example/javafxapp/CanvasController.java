...
public class CanvasController extends Application {
  ...
  final Map<Line, Arrow> connectionMap = new HashMap<>();
  ...
  private Circle currentSourcePort = null;
  private Line tempLine = null;

  // Обработчики для addEventFilter
  ...
  public void start(Stage primaryStage) {
    ...
    centerPane.getChildren().addAll(canvasLayer, shapeLayer, linesLayer, bubbleLayer);
  ...
  private void createLayers() {
    ...
    linesLayer = new Pane();
    linesLayer.setPickOnBounds(false);
  ...
  private VBox createToolBar() {
    ...
    Label shapeLabel2 = new Label("Блок-схема");
    shapeLabel2.setStyle("-fx-font-weight: bold; -fx-text-fill: #2c3e50;");

    Button rectBtn = new Button("Прямоугольник");
    rectBtn.setOnAction(e -> addNode(EditableNode.NodeType.RECTANGLE, "Действие", 200, 200));

    Button rhombusBtn = new Button("Ромб");
    rhombusBtn
        .setOnAction(e -> addNode(EditableNode.NodeType.RHOMBUS, "Проверка условия", 400, 200));

    Button ellipseBtn = new Button("Эллипс");
    ellipseBtn.setOnAction(e -> addNode(EditableNode.NodeType.ELLIPSE, "Начало/конец", 300, 400));

    Button trap2Btn = new Button("Перевёрнутая трапеция");
    trap2Btn.setOnAction(
        e -> addNode(EditableNode.NodeType.INVERTED_TRAPEZOID, "Ручная операция", 400, 300));

    Button btnParal = new Button("Параллелограмм");
    btnParal.setOnAction(
        e -> addNode(EditableNode.NodeType.PARALLELOGRAM, "Ввод/вывод данных", 200, 200));

    Button btnTrapezoid = new Button("Трапеция");
    btnTrapezoid
        .setOnAction(e -> addNode(EditableNode.NodeType.TRAPEZOID, "Запуск цикла", 200, 300));

    Button btnRoundRect = new Button("Скруглённый прямоугольник");
    btnRoundRect.setOnAction(e -> addNode(EditableNode.NodeType.ROUNDED_RECT, "Процесс", 200, 400));

    Button btnHexagon = new Button("Шестиугольник");
    btnHexagon
        .setOnAction(e -> addNode(EditableNode.NodeType.HEXAGON, "Подготовка цикла", 200, 500));
    ...
    toolBar.getChildren().addAll(
        ...
        selectTool);
    toolBar.getChildren().addAll(shapeLabel2, rectBtn, rhombusBtn, ellipseBtn, trap2Btn, btnParal,
        btnTrapezoid, btnRoundRect, btnHexagon, saveBtn, loadBtn);

    return toolBar;
  }

  public void addNode(EditableNode.NodeType type, String text, double x, double y) {
    EditableNode node = EditableNode.createNode(type, text);
    node.setLayoutX(x);
    node.setLayoutY(y);
    node.setOnPortDragStarted(this::startConnection);
    shapeLayer.getChildren().add(node);
  }

  private void startConnection(Circle sourcePort) {
    connectionMode = true;
    ...
    EditableNode sourceNode = (EditableNode) sourcePort.getParent();
    double startX = sourceNode.getLayoutX() + sourcePort.getCenterX();
    double startY = sourceNode.getLayoutY() + sourcePort.getCenterY();
    tempLine.setStartX(startX);
    tempLine.setStartY(startY);
    tempLine.setEndX(startX);
    tempLine.setEndY(startY);

    linesLayer.getChildren().add(tempLine);

    Scene scene = linesLayer.getScene();
    if (scene != null) {
      dragHandler = this::handleConnectionDrag;
      releaseHandler = this::handleConnectionRelease;
      scene.addEventFilter(MouseEvent.MOUSE_DRAGGED, dragHandler);
      scene.addEventFilter(MouseEvent.MOUSE_RELEASED, releaseHandler);
    }

    sourceNode.setConnecting(true);
  }

  private void handleConnectionDrag(MouseEvent event) {
    if (tempLine != null) {
      Point2D mouseInScene = new Point2D(event.getSceneX(), event.getSceneY());
      Point2D mouseInLines = linesLayer.sceneToLocal(mouseInScene);
      tempLine.setEndX(mouseInLines.getX());
      tempLine.setEndY(mouseInLines.getY());
    }
    event.consume();
  }

  private void handleConnectionRelease(MouseEvent event) {
    Scene scene = linesLayer.getScene();
    if (scene != null && dragHandler != null) {
      scene.removeEventFilter(MouseEvent.MOUSE_DRAGGED, dragHandler);
    ...
    try {
      if (currentSourcePort != null && tempLine != null) {
        Circle targetPort = findPortUnderCursor(event.getSceneX(), event.getSceneY());
  ...
  private void finishConnection(Circle targetPort) {
    if (targetPort == null || targetPort == currentSourcePort)
      return;
    ...
    bindPortLine(line, currentSourcePort, true);
    bindPortLine(line, targetPort, false);

    Arrow arrow = new Arrow(line, 12, 8);
    linesLayer.getChildren().add(line);
    linesLayer.getChildren().add(arrow);
    connectionMap.put(line, arrow);
  }

  private void bindPortLine(Line line, Circle port, boolean isStart) {
    EditableNode node = (EditableNode) port.getParent();

    if (isStart) {
      line.startXProperty()
          .bind(Bindings.createDoubleBinding(() -> node.getLayoutX() + port.getCenterX(),
              node.layoutXProperty(), node.layoutYProperty(),
              port.centerXProperty(), port.centerYProperty()));
      line.startYProperty()
          .bind(Bindings.createDoubleBinding(() -> node.getLayoutY() + port.getCenterY(),
              node.layoutXProperty(), node.layoutYProperty(),
              port.centerXProperty(), port.centerYProperty()));
    } else {
      line.endXProperty()
  ...
  private Circle findPortUnderCursor(double sceneX, double sceneY) {
    for (Node node : shapeLayer.getChildren()) {
      if (node instanceof EditableNode) {
        EditableNode editable = (EditableNode) node;
        Circle port = editable.findPortAtScene(sceneX, sceneY);
        if (port != null)
          return port;
      }
    }
  ...
  private void cleanupConnection() {
    if (tempLine != null) {
      linesLayer.getChildren().remove(tempLine);
      tempLine = null;
    }
    if (currentSourcePort != null) {
      EditableNode sourceNode = (EditableNode) currentSourcePort.getParent();
      sourceNode.setConnecting(false);
    }
  ...
  public void saveToFile(String filePath) {
    ...
    for (javafx.scene.Node node : shapeLayer.getChildren()) {
      if (node instanceof EditableNode) {
        ...
        nodeObj.put("type", editable.getNodeType().name()); // нужно добавить геттер в EditableNode
        nodeObj.put("text", editable.getText());
        nodeObj.put("x", editable.getLayoutX());
        nodeObj.put("y", editable.getLayoutY());
    ...
    for (javafx.scene.Node node : linesLayer.getChildren()) {
      if (node instanceof Line) {
        Line line = (Line) node;
        // Ищем стрелку, которая привязана к этой линии (Arrow)
        Arrow arrow = findArrowForLine(line);
        if (arrow == null)
          continue; // временная линия или линия без стрелки

        // Ищем исходный и целевой порты по координатам
        Circle sourcePort = findPortAt(line.getStartX(), line.getStartY());
        Circle targetPort = findPortAt(line.getEndX(), line.getEndY());
        if (sourcePort == null || targetPort == null)
          continue;

        EditableNode sourceNode = (EditableNode) sourcePort.getParent();
        EditableNode targetNode = (EditableNode) targetPort.getParent();
        ...
        connObj.put("sourceNodeIndex", sourceIndex);
        connObj.put("sourcePort", sourceNode.getPortName(sourcePort));
        connObj.put("targetNodeIndex", targetIndex);
        connObj.put("targetPort", targetNode.getPortName(targetPort));
    ...
    try (FileWriter writer = new FileWriter(filePath)) {
      writer.write(root.toString(4)); // 4 пробела для отступов
    } catch (Exception e) {
      logger.log(Level.SEVERE, "Ошибка", e);
    }
  }
  ...
  public void loadFromFile(String filePath) {
    try {
      ...
      for (int i = 0; i < nodesArray.length(); i++) {
        ...
        EditableNode.NodeType type = EditableNode.NodeType.valueOf(typeStr);
        String text = nodeObj.getString("text");
      ...
      JSONArray connectionsArray = root.getJSONArray("connections");
      for (int i = 0; i < connectionsArray.length(); i++) {
        JSONObject connObj = connectionsArray.getJSONObject(i);
        int sourceIndex = connObj.getInt("sourceNodeIndex");
        String sourcePortName = connObj.getString("sourcePort");
        int targetIndex = connObj.getInt("targetNodeIndex");
        String targetPortName = connObj.getString("targetPort");

        EditableNode sourceNode = getNodeByIndex(sourceIndex);
        EditableNode targetNode = getNodeByIndex(targetIndex);
        if (sourceNode == null || targetNode == null)
          continue;

        Circle sourcePort = sourceNode.getPortByName(sourcePortName);
        Circle targetPort = targetNode.getPortByName(targetPortName);
        if (sourcePort == null || targetPort == null)
          continue;

        // Создаём линию и привязываем
        Line line = new Line();
        line.setStroke(Color.BLACK);
        line.setStrokeWidth(2);
        bindPortLine(line, sourcePort, true);
        bindPortLine(line, targetPort, false);
        Arrow arrow = new Arrow(line, 12, 8);
        linesLayer.getChildren().addAll(line, arrow);
      }
    } catch (Exception e) {
      logger.log(Level.SEVERE, "Ошибка", e);
    }
  ...
  private Arrow findArrowForLine(Line line) {
    return connectionMap.get(line);
  }

  // Индекс фигуры в слое
  private int getNodeIndex(EditableNode node) {
    for (int i = 0; i < shapeLayer.getChildren().size(); i++) {
      if (shapeLayer.getChildren().get(i) == node)
        return i;
    }
    return -1;
  }
  ...
  // Фигура по индексу
  private EditableNode getNodeByIndex(int index) {
    if (index >= 0 && index < shapeLayer.getChildren().size()) {
      return (EditableNode) shapeLayer.getChildren().get(index);
    }
    return null;
  }

  // Найти порт по координатам (приблизительно)
  private Circle findPortAt(double x, double y) {
    for (javafx.scene.Node node : shapeLayer.getChildren()) {
      if (node instanceof EditableNode) {
        EditableNode editable = (EditableNode) node;
        for (Circle port : editable.getAllPorts()) {
          double portX = editable.getLayoutX() + port.getCenterX();
          double portY = editable.getLayoutY() + port.getCenterY();
          if (Math.abs(portX - x) < 5 && Math.abs(portY - y) < 5) {
            return port;
          }
        }
      }
  ...
  private void saveDrawing() {
    ...
    try {
      ...
      Pane oldParentLine = (Pane) linesLayer.getParent();
      ...
      int lineIndex = oldParentShape != null ? oldParentShape.getChildren().indexOf(linesLayer)
          : -1;

      Group snapshotGroup = new Group();
      snapshotGroup.getChildren().addAll(canvasLayer, shapeLayer, linesLayer);
      ...
      if (oldParentLine != null) {
        if (lineIndex >= 0 && lineIndex <= oldParentLine.getChildren().size()) {
          oldParentLine.getChildren().add(lineIndex, linesLayer);
        } else {
          oldParentLine.getChildren().add(linesLayer);
        }
      }
